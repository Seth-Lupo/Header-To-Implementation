<script lang="ts">
    ...
    import implementationTextStore from '../lib/implementationTextStore'
    import headerTextStore from '$lib/headerTextStore';

    let implementationText : string;

    
    implementationTextStore.subscribe(text => {
        implementationText = text;
    });

    let copyText = () => {
        navigator.clipboard.writeText(implementationText);
    }
...
<main class="ioBlock">
    <div class="headerDiv">
        <h3>Implementation Scaffold</h3>
        <button class="copyButton" on:click={copyText}></button>
    </div>
    <textarea>{implementationText}</textarea>
  </main>
